(function() {
  ...
  if (window.location.hostname === 'lumea-ar.com' ||
      ...
      if (typeof LS !== 'undefined' && LS.cart && LS.cart.id) {
        ...
        if (cartId && storeId && !window.location.href.includes('success')) {
          ...
          if (!sentRequests[requestId]) {
            // Elegir endpoint según dominio de la tienda
            var hostname = window.location.hostname;
            var endpoint;
            if (hostname === 'www.fuled.com.ar' || hostname === 'fuled.com.ar') {
              endpoint = 'https://alerti-backend.vercel.app/api/checkout';
            } else {
